...
Two layers, mirroring test_gates.py:
  * Gate unit tests exercise app.quality.gates.check_facts as the pure
    function it is — a synthetic facts-v1 dict + synthetic DB inputs, no
...
# ---------------------------------------------------------------------------
# Synthetic facts-v1 fixture (a dict that passes every gate check)
# ---------------------------------------------------------------------------
        ...
        "meetings": [
            {
                ...
                "disciplinary": [
                    {"category": "revocation", "count": 2,
                     "quote": "two licenses were revoked",
                     "source_document": "may.pdf", "confidence": "high"},
                    {"category": "probation", "count": 1,
...
def test_single_quote_mismatch_downgrades_confidence():
    data = _good_data()
    # break ONE of the four quotes (1/? mismatch stays under 30%).
    data["meetings"][0]["legislation"][0]["quote"] = "not a real substring"
    result = check_facts("XX_MD", data, **_gate_kwargs())
    assert result.ok  # under the 30% file-level threshold
...
def test_quote_mismatch_rate_over_30pct_rejects():
    data = _good_data()
    # break the policy + legislation + one disciplinary quote = 3/4 = 75%.
    data["meetings"][0]["policy_actions"][0]["quote"] = "fake one"
    data["meetings"][0]["legislation"][0]["quote"] = "fake two"
    data["meetings"][0]["disciplinary"][0]["quote"] = "fake three"
    result = check_facts("XX_MD", data, **_gate_kwargs())
    assert not result.ok
...
async def test_disciplinary_category_replaced_not_duplicated(facts_db):
    from app.extractor.facts import ingest_facts_file
    db_file, out_dir, _ = facts_db

    # First ingest: revocation=2.
    ingest_facts_file(_write_facts_file(out_dir, _good_data()))

    # Second ingest: same meeting, revocation now =5 (a corrected count).
    data = _good_data()
    data["meetings"][0]["disciplinary"][0]["count"] = 5
    ingest_facts_file(_write_facts_file(out_dir, data))

    con = sqlite3.connect(db_file)
    try:
        rows = con.execute(
            "SELECT category, action_count FROM disciplinary_actions "
            "WHERE category='revocation'").fetchall()
    finally:
        con.close()
    # UNIQUE(meeting_id, category): one row, the replacement count.
    assert rows == [("revocation", 5)]
