...
async def discipline_trends(today: date | None = None) -> dict:
    """Disciplinary action counts from v_disciplinary.

    Two views:
      * quarterly_stacked: total actions per outcome category by quarter over
        the last 8 quarters (a stacked column series per category).
      * per_state_12mo: total actions per state over the trailing 12 months.
    ...
    async with db.async_session() as session:
        ...
        state_rows = (await session.execute(text("""
            SELECT state, SUM(action_count) AS n
            FROM v_disciplinary
            WHERE meeting_date <= date('now')
              AND meeting_date >= date('now', '-12 months')
            GROUP BY state
            ORDER BY n DESC
        """))).all()
    ...
    for quarter, category, n in cat_rows:
        if quarter not in q_set:
            continue
        by_cat.setdefault(category, {})[quarter] = n

    category_series = []
    for category in sorted(by_cat):
        data = [by_cat[category].get(q, 0) for q in quarters]
        category_series.append({"name": category.replace("_", " "), "data": data})

    per_state = [{"state": s, "count": n} for s, n in state_rows]
