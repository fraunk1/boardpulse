...
def main():
    ...
    bucketed = sum(map(len, (have_docs, accounted, mtgs_no_docs,
                             url_no_mtgs, no_url)))
    assert bucketed == total, f"bucket leak: {bucketed} != {total}"

    _dump("HAVE DOCS", have_docs)
    _dump("ACCOUNTED (none_published / blocked)", accounted)
    _dump("MEETINGS BUT NO DOCS", mtgs_no_docs)
    _dump("URL SET, NO MEETINGS FOUND", url_no_mtgs)
    _dump("NO URL (undiscovered)", no_url)
